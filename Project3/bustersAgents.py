        ...
        for ghost in livingGhostPositionDistributions:
            ...
            for pos, prob in ghost.items():
                if prob > ghostProb:
                    ghostPos = pos
                    ghostProb = prob
            maxPositions.append(ghostPos)

            shortDist = 999
            closeGhost = None
            for position in maxPositions:
                dist = self.distancer.getDistance(pacmanPosition, position)
                if shortDist > dist:
                    shortDist = dist
                    closeGhost = position
            ...
            for action in legal:
                ...
                newDist = self.distancer.getDistance(closeGhost, successorPosition)
                if shortNewDist > newDist:
                    shortNewDist = newDist
                    bestAction = action
