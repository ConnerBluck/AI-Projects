...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is None:
            ...
            for p in self.legalPositions:
                trueDistance = util.manhattanDistance(p, pacmanPosition)
                allPossible[p] = emissionModel[trueDistance] * belief[p]
            if allPossible.totalCount() == 0:
                self.initializeUniformly(gameState)
            else:
                allPossible.normalize()
                for num in range(self.numParticles):
                    randomSample = util.sample(allPossible)
                    newParticles.append(randomSample)
                self.particles = newParticles
    ...
    def observeState(self, gameState):
        ...
        ** Remember ** We store particles as tuples, but to edit a specific particle,
        ...
        for particle in self.particles:
            ...
            for num in range(self.numGhosts):
                if noisyDistances[num] is not None:
                    trueDistance = util.manhattanDistance(particle[num], pacmanPosition)
                    weight *= emissionModels[num][trueDistance]
                else:
                    particle = list(particle)
                    particle[num] = self.getJailPosition(num)
        ...
        if allPossible.totalCount() == 0:
            ...
            for particle in self.particles:
                for num in range(self.numGhosts):
                    if self.particles[num] is None:
                        particle = self.getParticleWithGhostInJail(particle, num)
        else:
            for num in range(self.numParticles):
                randomSample = util.sample(allPossible)
                newParticles.append(randomSample)
    ...
    def elapseTime(self, gameState):
        ...
        **Note** that you may need to replace "prevGhostPositions" with the
        ...
        for oldParticle in self.particles:
            ...
            "*** YOUR CODE HERE ***"
            for i in range(self.numGhosts):
                newPosDist = getPositionDistributionForGhost(setGhostPositions(gameState, newParticle), i,
                                                             self.ghostAgents[i])
